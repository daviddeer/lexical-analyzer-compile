...
int lookup(char *token)
{
	...
	for(j=1;j<=norw-1;j++)if(strcasecmp(token,word[j])==0) return j;   //以TOKEN字符串查保留字表，若查到返回保留字类别码
